...
| 15 | [`langsonzhang/Terminal-C1-Midwest-2022`](https://github.com/langsonzhang/Terminal-C1-Midwest-2022) ("Murphy's Lawyers," UofT) | Spring 2022, C1 Midwest (hosted by Citadel) | GitHub repo | Built the "FUNNEL" algorithm; placed **#5 of 24 teams** against CMU/UMich/UIUC competitors | Verified (self-reported placement); README does not describe the algorithm's internals beyond the name | **Milestone 6 follow-up (Jul 2026): actually benchmarked this round** as `public_opponents/funnel_uoft` (723-line `algo_strategy.py` + 4 supporting modules, real/non-stub, standard starter-kit `License.md`, no separate license for their own code — used strictly as an unmodified black-box opponent). **Config mismatch, checked directly**: their shipped config is the old `seasonCompatibilityMode: 5` schema — Wall `startHealth: 60.0` (vs our `40.0`), Support `startHealth: 1.0` unshielded / upgrade `shieldPerUnit: 1` (vs our `2.0`/`4`) — their Support was apparently *much* squishier and *less* shield-potent pre-upgrade than ours, which may partly explain the result below. **Result: `milestone5-champion` WON 10/10, both seats, no crashes, decisively (mean 10 turns, 31-0 points_scored in the observed game)** — the fastest, most lopsided win of any independent opponent tested in this whole project; see `docs/REAL_OPPONENT_RESULTS.md` §7 |
| 16 | [`yip6ga1lok6/C1-Terminal-Summer-2022`](https://github.com/yip6ga1lok6/C1-Terminal-Summer-2022) | Summer 2022 Invitational | GitHub repo | Ranked **6th of 91 teams** | Verified (self-reported placement) | **Milestone 6 follow-up (Jul 2026): actually benchmarked this round** as `public_opponents/summer2022_6th` (752-line `algo_strategy.py`, real/non-stub, standard starter-kit `License.md`, no separate license for their own code — used strictly as an unmodified black-box opponent; repo also ships `rust-algo`/`java-algo` variants of unclear relative strength/finality — python-algo is the one actually run here, noted as an honest ambiguity, not resolved). **Config mismatch, checked directly**: their shipped config is also the old `seasonCompatibilityMode: 5` schema — Wall `startHealth: 12.0` (vs our `40.0`, much squishier), Turret `attackDamageWalker: 16.0`/`cost1: 6.0` (vs our base `5.0`/`2.0` — their base Turret hits much harder but costs 3x more). **Result: `milestone5-champion` WON 10/10, both seats, no crashes (mean 56 turns — a much more substantial, drawn-out engagement than #15 above, but still a clean sweep)**; see `docs/REAL_OPPONENT_RESULTS.md` §7 |
...
| 21 | Web search for named winners' own code (`Smite`/Stanford, `QY`/Cambridge, `Garpuz`/UW, `Bin Birds`/APAC 2026 — sources #10, #13, #14, #18 above) | Milestone 6 follow-up (Jul 2026) | Negative-result search | **No public repo found for any of these four named winning teams.** `luckystarufo`'s repo (#7) already documented the "please don't publish" norm directly; this follow-up specifically checked whether that norm actually held for our *most* relevant/recent named winners and confirmed it does. For "Bin Birds" specifically: identified the actual team members (Lucas Liang, Hugo Jasmin, Trong Nghia Nguyen) via Lucas Liang's own LinkedIn post confirming the win, then checked his linked personal GitHub (`lucasisntcoding`) directly — 12 public repos, all quant-finance/web projects, **zero** Terminal/C1Games code | Verified (direct GitHub account check, not just a search-engine miss) | Confirms caveat #2 still holds for the single most recent/relevant winner in this whole report; closes this specific research avenue rather than leaving it open-ended |
| 22 | [`satvikmittal638/Terminal-2026-Skill-Issue`](https://github.com/satvikmittal638/Terminal-2026-Skill-Issue) (Team "Skill_Issue": Satvik Mittal, Om Gore, Chatanya Maheshwari) | Created Mar 2026, "Citadel Terminal Competition" (repo's own description) | GitHub repo, complete/runnable | Placed **3rd of 4** shown on their own posted leaderboard image, but reports the **highest Elo (2357)** of the four listed teams (1st: HUNTR/X 2053, 2nd: BBH 1846, 3rd: Skill_Issue 2357, 4th: Segmentation_Fault 2275) — Elo and placement don't move together in their own table, unexplained by anything in the repo. Strategy: never buys Walls, never upgrades Turrets; relies on 4 upgraded Supports + phased reactive Turret walls; offense is exclusively "chunks of 5" Scouts, alternating sides, explicitly optimizing for the sub-40ms compute-time tie-break | Verified (real repo, 235-line non-stub `algo_strategy.py`, explicit strategy writeup in-repo) | **Config mismatch, checked directly**: their shipped `game-configs.json` has `shieldRange: 0` for Support (i.e. the OLD, since-corrected no-op shield this project itself was built against pre-Milestone-4) plus `generatesResource1/2` fields our current config doesn't have at all, Turret `attackRange: 2.5` (vs our `4.5`), Demolisher `cost2: 3.0` (vs our `2.0`), `startingHP: 40.0` (vs our `30.0`). Their "never upgrade Turret, only Support" philosophy was very plausibly tuned against an economy where Support's shield was a real no-op too (like ours pre-M4) — used here strictly as an unmodified black-box test opponent, see `docs/MILESTONE_6_REPORT.md` for the actual benchmark (we won 20/0) |
| 23 | [`The-Travelling-Salesmen/terminal-c1`](https://github.com/The-Travelling-Salesmen/terminal-c1) | Created Feb 2020 | GitHub repo, complete/runnable, **multiple iterations preserved** | Repo's own description: "Home of the sweet, sweet algo that helped us clinch #1 spot at Harvard in Terminal." Contains ~10 algo folders spanning an iteration history (`snorkeldink-v1` → `v2` → `v3-1/v3-2/v3-3` → `AdapDef`, plus `frumblesnatch-v1`). **Important correction made during vetting**: the more official-sounding `AdapDef` folder is actually mostly *unmodified starter-kit boilerplate* (retains starter debug strings, unused starter helper methods) and, empirically, LOSES to our champion (20/0). The actual strongest, most-evolved iteration is `snorkeldink-v3-3`, whose own docstring self-declares `"Strategy-code for the final version of Snorkeldink-V69"` — this is the one that beats our champion (0/20, see `docs/MILESTONE_6_REPORT.md`). Core mechanism: adaptive wall-opening (attacks whichever side has less Filter+Destructor "badness" score), then a continuous, un-paused, 100%-MP-committed Demolisher stream at that lane every turn from turn 4 onward — no bursts, no pauses, ever | Verified (real repo, real git history, `v3-3`'s 261+73+43-line non-stub code); **Verified the file-name/"best version" assumption was WRONG on first pass and corrected it before finalizing this report** | **Config mismatch, checked directly**: their shipped config predates the current schema entirely (no `resources` block/`startingHP` field at all — an even older season format than Skill_Issue's), Turret `cost1: 6.0` (vs our `2.0`) and upgrade `attackDamageWalker: 32.0` (vs our `16.0`), Support `shieldRange: 3.5` (vs our `2.5`, but real/non-zero in both). This bot's raw stat assumptions are the most out-of-date of any candidate found — its real historical strength could be under- *or* over-stated by our config, a genuine two-sided caveat, not just a reason to discount the result |
| 24 | [`davidw0311/c1_terminal`](https://github.com/davidw0311/c1_terminal) | Repo created Oct 2023, pushed as recently as Mar 2026 | GitHub repo, complete/runnable | 898/971-line MCTS-based algo (`numpy`/`scipy`), tracks enemy spawns via `on_action_frame`, no publicly claimed competition placement (appears to be a personal/student project, also the same repo already used in `docs/REAL_OPPONENT_RESULTS.md` §1 as the only non-official source even mentioning the practice-bot "Ironclad") | Verified (real, substantial, non-stub code); no license file, no placement claim | No `game-configs.json` shipped in the repo to directly diff against ours; its MCTS simulation logic reads config values from whatever the engine hands it at runtime rather than hardcoding stats, so a config mismatch here is much less of a concern than for #22/#23 — but its rollout budget/depth was very likely originally tuned against a different config's economy. Benchmark: won 20/0 against it (`docs/MILESTONE_6_REPORT.md`); notably its own compute time was ~131-160s **per game** (vs our ~200ms), i.e. ~2600-4600ms/turn on average across a 35-99 turn game — well under the 5000ms soft/35000ms hard per-turn caps but the slowest opponent this project has benchmarked against, worth keeping in mind for any future compute-budget comparison |
| 25 | Second, upgraded-goal re-check specifically for former **world/global champions'** own code (Milestone 6 follow-up round 2, per user's explicit request to prioritize the *strongest* possible opponent): re-searched Smite/Stanford, QY/Cambridge, Garpuz/UW directly (already covered in #21) **plus** a newly-found, independently-verified real champion not previously named in this report — **"Lee Isaac," 2022 Citadel Terminal Summer Invitational Champion (Rank 1/42, $6,500 prize)**, per their own LinkedIn credential list, which also separately lists a **peak Season 8 rank of 1st out of 800+ players** | Jul 2026 (this search round) | Negative-result search (named individual) | **No public repo found.** The LinkedIn profile links a GitHub account (`Lee-Isaac`), but that account belongs to a different, unrelated person by the same name (a DevOps engineer in Seoul with 60 unrelated repos — Web-Roulette, Java course projects, etc. — zero Terminal/C1Games code). No other GitHub account could be tied to this specific champion | Verified (direct GitHub account check found a name-collision, not the actual champion) | A second, independently-found real champion (not one of the four already checked in #21) confirms the "don't publish complete algos" norm yet again — this is now the *fifth* separately-checked named champion/high-placer with zero published code, making this pattern very well-established rather than a one-off |
| 26 | [`wllmzhu/alpha-terminal`](https://github.com/wllmzhu/alpha-terminal) (already logged in #8 for its stat-mismatch finding) | Milestone 6 follow-up (Jul 2026): re-examined specifically for runnability this round | GitHub repo, real training code, **not actually benchmarkable as-is** | Policy-gradient/LSTM RL agent requires `torch` (not installed in this environment, but installable) — **however, the repo ships no trained checkpoint file (`.pth`) at all**, only the model architecture and training loop. Its own `CheckpointManager` falls back to a freshly, randomly-initialized `PolicyNet` when no checkpoint exists on disk. Running it as-is would therefore only test an **untrained, random-weight policy network** — not the actual "strong amateur" agent described in the repo's own linked report | Verified (direct inspection of `utils.py`'s `CheckpointManager.checkpoint_exists()`/`get_checkpoint_path()`, and confirmed via `find` that no `.pt`/`.pth`/checkpoint file exists anywhere in the cloned repo) | **Correctly excluded from benchmarking, not because it's a dud/stub (the code itself is real and substantial — 283-line `algo_strategy.py` + a small `arch/` package), but because a meaningful benchmark here is impossible without the actual trained weights**, which were never published alongside the code. Benchmarking a randomly-initialized network and reporting it as representative of "an RL agent's real strength" would be actively misleading — this is exactly the kind of honest exclusion this project's discipline calls for, distinct from the "untouched starter-kit fork" dud category (`vinharish77/TerminalCompetition`, re-confirmed as a dud again this round) |

**Milestone 6 update — the single most load-bearing finding across every entry in this table**: source #23's `snorkeldink-v3-3` is a real, substantial, publicly-sourced, black-box-tested opponent (never copied, only run as an unmodified test opponent locally) that **beats `milestone5-champion` 0/20, both seats, no crashes** — the first genuinely independent opponent (as opposed to anything self-built by this project) to do so. Full root-cause analysis, five separate patch attempts (all of which delayed but did not close the loss), and the honest final recommendation are in `docs/MILESTONE_6_REPORT.md`. This is flagged here in the prior-art report, not just in the milestone report, because it directly validates this report's own §5 recommendation #6 ("maze/funnel defensive layouts... benchmark our own baseline's breach rate") and #4 ("explicitly test the health-preservation vs. damage-race trade-off") from the *opposite* side: a sustained, un-paused, single-lane rush is apparently still a viable, currently-unsolved-by-us strategy under the corrected config, not just a historical 2019-era pattern.
...
- **No independent replay analysis was performed by this research workstream** — I
  ...
  **Partially closed, Milestone 6:** the engineering workstream has now run several
  of these named/sourced repos (#22-#24) as actual local black-box opponents against
  our own champion, with full replay-level root-cause analysis of the one real loss
  found — see `docs/MILESTONE_6_REPORT.md`. This is real signal for *our specific
  champion's* current weaknesses, not a general replacement for this report's own
  self-reported-postmortem evidence base, but it is the first place in this whole
  project where a prior-art-sourced opponent's code was actually run, not just read
  about.
