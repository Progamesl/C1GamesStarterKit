...
"""
CANDIDATE: "defense_v8_offense_burst" -- defense_v6_encryptor_fix PLUS a
normal-game offense improvement (Milestone 6, docs/MILESTONE_6_REPORT.md
section 2-3).

Milestone 6 honest gap analysis (Verified by direct code reading, not
inference -- see docs/MILESTONE_6_REPORT.md section 2 for the full
cross-reference against docs/STRATEGIC_PRIOR_ART_REPORT.md): every one of
this repo's documented offensive prior-art hypotheses (escorted combined-arms
§3.3, turn-level minimax §3.2, signature detection §3.5, opponent prediction
§3.7) had, until this milestone, only ever been built as OPPONENTS to test
our OWN defense against -- none had been incorporated into our own champion's
actual offensive logic. `defense_v6_encryptor_fix`'s normal-game offense
(`opportunistic_offense`, i.e. everything outside the three endgame modes)
was: dump unlimited SCOUTs, scouts only, every other turn, once MP>=9 -- no
composition choice, and a floor low enough that it rarely represented
genuinely *accumulated* MP rather than that turn's raw income.

THE FIX (this candidate only -- defense_v6_encryptor_fix is left completely
unmodified as the still-current champion and rollback point):
`opportunistic_offense` now uses a real MP-threshold burst
(`OFFENSE_MP_BURST_THRESHOLD=15`, meaningfully higher than the old floor of
9, so MP has to genuinely accumulate across several turns of income first)
and a real composition split (`OFFENSE_DEMOLISHER_MP_FRACTION=0.35` of the
accumulated MP goes to DEMOLISHER -- the only mobile unit that reliably
damages structures at range -- alongside the SCOUT screen, instead of scouts
only). SCOUT's speed 1 vs. DEMOLISHER's speed 0.5 means the scouts still
naturally arrive first and draw fire even when spawned the same turn, giving
an implicit escort without needing an explicit multi-turn stagger. Turn-parity
gating is dropped entirely -- the threshold itself paces the cadence, since MP
has to refill to the threshold again after each burst.

--- Everything below is unchanged from defense_v6_encryptor_fix (this
candidate's parent) ---
...
# Milestone 6 offense improvement (see opportunistic_offense docstring +
# docs/MILESTONE_6_REPORT.md section 2-3 for the honest gap analysis this is
# based on): normal-play offense used to be "dump unlimited scouts every
# other turn once MP>=9" -- no composition choice at all, and a floor so low
# it triggered almost every eligible turn rather than ever really
# accumulating. Replaced with a real resource threshold (higher than the old
# floor, so MP actually has to accumulate across a few turns first, same
# accumulate-then-burst idea as opponents/adaptive_burst_combined_arms) and a
# real composition choice (a DEMOLISHER structure-cracker fraction alongside
# the SCOUT screen, not scouts-only) -- i.e. the escorted-combined-arms
# mechanism (docs/STRATEGIC_PRIOR_ART_REPORT.md 3.3), which was previously
# only ever built as OPPONENTS to test our own defense, never incorporated
# into our own offense outside the rare stalemate-breaker/endgame branches.
OFFENSE_MP_BURST_THRESHOLD = 15
OFFENSE_DEMOLISHER_MP_FRACTION = 0.35
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def on_game_start(self, config):
        gamelib.debug_write('Configuring DEFENSE_V8_OFFENSE_BURST candidate...')
        self.config = config
    ...
    def on_turn(self, turn_state):
        ...
        gamelib.debug_write('DEFENSE_V8 turn {} my_hp={} enemy_hp={} mode={}'.format(
    ...
    def opportunistic_offense(self, game_state):
        ...
        Compute-time fix (see module docstring): the lane choice is cached and
        ...
        Milestone 6 offense improvement (docs/MILESTONE_6_REPORT.md section 2-3):
        this used to be "dump unlimited scouts every other turn once MP>=9" --
        no real accumulation (9 MP is close to what a turn's raw income alone
        can hit within a few turns, so this rarely meant genuinely *saved-up*
        MP) and no composition choice at all (scouts only; DEMOLISHER, the only
        mobile unit that reliably damages structures at range, never appeared
        in normal-game offense, only in the rare stalemate-breaker/endgame
        branches). Replaced with a real MP-threshold burst (OFFENSE_MP_BURST_THRESHOLD,
        higher than the old floor, so MP has to genuinely accumulate across
        several turns of income before triggering) and a real composition split
        (a DEMOLISHER fraction of the accumulated MP alongside the SCOUT
        screen -- SCOUT's speed 1 vs. DEMOLISHER's speed 0.5 means the scouts
        still naturally arrive first and draw fire even spawned the same turn,
        the same escorted-combined-arms mechanism this repo has only ever
        built as OPPONENTS until now, docs/STRATEGIC_PRIOR_ART_REPORT.md 3.3).
        Turn-parity gating is dropped in favor of the threshold itself pacing
        the cadence -- MP has to refill to OFFENSE_MP_BURST_THRESHOLD again
        after each burst, which naturally spaces bursts out over multiple
        turns without an explicit modulo check.
        """
        turn = game_state.turn_number
        if turn < 6:
        ...
        mp = game_state.get_resource(MP)
        if mp >= OFFENSE_MP_BURST_THRESHOLD:
            best = self._get_cached_lane(game_state)
            demolisher_cost = game_state.type_cost(DEMOLISHER)[MP]
            demolisher_budget = mp * OFFENSE_DEMOLISHER_MP_FRACTION
            num_demolishers = int(demolisher_budget // demolisher_cost)
            if num_demolishers > 0:
                game_state.attempt_spawn(DEMOLISHER, best, num_demolishers)
            game_state.attempt_spawn(SCOUT, best, 1000)
