...
"""
CANDIDATE: "defense_v5_demolisher_corner" -- Milestone 4 experimental tactic #2
targeting the structural turtle_survivor weakness (defense_v4_tiebreak plateaus at
~15%, 3/20, see docs/MILESTONE_3_REPORT.md). Same root-cause finding as
defense_v5_bigwave (docs/MILESTONE_4_REPORT.md): the first wall is free to break
(zero turret coverage at the corner pathfinding funnels everyone to), the second
wall one tile in brings exactly 1 turret's fire. This variant changes WAVE
COMPOSITION instead of pacing: DEMOLISHER deals 6.0 structure damage per hit vs
SCOUT's 2.0 (3x), so a demolisher-heavy wave should crack the second wall layer in
fewer hits (fewer frames exposed to the single covering turret) per MP spent on
front-line breachers, escorted by a lighter scout screen to add extra simultaneous
bodies for the turret to have to choose between. Banks MP like defense_v5_bigwave
so the wave arrives as one simultaneous mass instead of a trickle.
...
BANK_MP_THRESHOLD = 24  # Milestone 4 tactic #2: bank until this much MP, then dump it all at once
OFFENSE_DEMOLISHER_MP_FRACTION = 0.6  # heavier than ALL_IN's 0.4 -- structure-breach focused
