...
"""
CANDIDATE: "defense_v5_bigwave" -- Milestone 4 experimental tactic #1 targeting the
structural turtle_survivor weakness (defense_v4_tiebreak plateaus at ~15%, 3/20, see
docs/MILESTONE_3_REPORT.md). Root-cause analysis (docs/MILESTONE_4_REPORT.md) found
turtle_survivor's front wall is FULL-WIDTH (x=0..27, no gap anywhere), so pathfinding
funnels every attacker to one of the two extreme board corners regardless of which
lane we nominally pick -- and that specific corner cell has ZERO turret coverage
(confirmed by direct get_attackers() probing of a reconstructed turtle_survivor
board), so breaking the FIRST wall there is free. The bottleneck is the SECOND wall
layer one tile further in, which brings exactly 1 turret's fire (vs 2 in the middle),
and defense_v4's every-2-turns moderate scout dump doesn't reliably send enough
simultaneous bodies to eat that one turret's fire long enough to also crack the
second wall before the wave fully dies. This variant ONLY changes offense pacing:
bank MP for BANK_TURNS turns doing nothing offensively, then unleash the ENTIRE
banked reserve as one single, much larger simultaneous scout wave, on the theory
that a bigger single burst has more survivors-in-flight when the second wall's lone
turret can only kill one body per frame.
...
BANK_MP_THRESHOLD = 24  # Milestone 4 tactic #1: bank until this much MP, then dump it all at once
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def on_game_start(self, config):
        gamelib.debug_write('Configuring DEFENSE_V4_TIEBREAK candidate...')
        self.config = config
    ...
    def on_turn(self, turn_state):
        ...
        gamelib.debug_write('DEFENSE_V4 turn {} my_hp={} enemy_hp={} mode={}'.format(
