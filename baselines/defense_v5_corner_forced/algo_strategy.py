...
"""
CANDIDATE: "defense_v5_corner_forced" -- Milestone 4 experimental tactic #3 (control)
targeting the structural turtle_survivor weakness (defense_v4_tiebreak plateaus at
~15%, 3/20, see docs/MILESTONE_3_REPORT.md). Offline board reconstruction
(docs/MILESTONE_4_REPORT.md) showed that BOTH of defense_v4's existing candidate
lanes ([13,0] and [3,10]) already get funneled by find_path_to_edge's idealness
search to one of the two extreme board corners (turtle_survivor's front wall is
full-width with no gap, so every path is blocked identically and the tie-break
resolves toward whichever corner is closest to the target direction) -- this
variant is a deliberate control to test that finding directly: it replaces the
lane *candidates* themselves with the literal edge corners ([0,13]/[27,13]) instead
of [13,0]/[3,10], to check whether explicitly targeting the corner (rather than
relying on incidental pathfinding funneling from a nominally different lane)
changes anything. Everything else (pacing, unit mix) is identical to
defense_v4_tiebreak.
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def _get_cached_lane(self, game_state):
        ...
        if self._cached_lane is None or (turn - self._cached_lane_turn) >= RECOMPUTE_INTERVAL:
            ...
            options = [[0, 13], [27, 13]]  # Milestone 4 tactic #3: literal board corners
