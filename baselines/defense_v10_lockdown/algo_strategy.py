...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def upgrade_core(self, game_state):
        ...
        # Milestone 7 fix (module docstring sec 2): while a side is in lockdown,
        # skip upgrading EVERYTHING except the contested cluster itself -- the
        # opposite corner cluster, the center anchors, AND the front wall row --
        # all of that SP is better spent on the contested side's rebuild via
        # reactive_defense's raised cap below, which is the actual fix for the
        # region-wide-collapse root cause (module docstring). The contested
        # side's OWN cluster keeps upgrading normally (checked directly, module
        # docstring: range loss doesn't actually lose coverage of the corner at
        # these distances, so this is a free damage upgrade against
        # non-Demolisher threats elsewhere in the opponent corpus, not
        # something to also pause).
    ...
    def opportunistic_offense(self, game_state):
        ...
        if self.lockdown_side is not None:
            # Milestone 7 fix (module docstring sec 2-3): once a side is in
            # lockdown, counter-offense becomes CONTINUOUS (every turn, not
            # gated to every 3rd turn / an MP-floor check) and commits a much
            # larger MP fraction -- this is the "match their sustained,
            # un-paused posture" half of the fix, not just passive defense.
            # Quantitatively checked (docs/MILESTONE_7_REPORT.md sec 2) that
            # this cannot work as a primary win condition on its own (the
            # opponent's SP-funded defense is untouched by MP pressure), but
            # it forces real, measurable losses along the way and is kept as
            # an honest secondary component.
            self._lockdown_counter_offense(game_state)
            return
    ...
    def _lockdown_counter_offense(self, game_state):
        best = self._get_cached_lane(game_state)
        mp = game_state.get_resource(MP)
        demolisher_cost = game_state.type_cost(DEMOLISHER)[MP]
        demolisher_budget = mp * LOCKDOWN_COUNTER_MP_FRACTION
        num_demolishers = int(demolisher_budget // demolisher_cost)
        if num_demolishers > 0:
            game_state.attempt_spawn(DEMOLISHER, best, num_demolishers)
        game_state.attempt_spawn(SCOUT, best, 1000)
