...
"""
CANDIDATE: "defense_v9_corner_depth" -- defense_v6_encryptor_fix (milestone5-champion)
PLUS a fix for a real weakness found in Milestone 6 against a genuinely independent,
publicly-sourced opponent (`public_opponents/travelling_salesmen_v33`, an iteration of
a team's algo that a repo README claims placed #1 at a Harvard Terminal event --
see docs/REAL_OPPONENT_RESULTS.md and docs/MILESTONE_6_REPORT.md for the full
sourcing/vetting/benchmark record).

Milestone 6 root-cause finding (Verified via turn-by-turn replay breach-event and
spawn/death-event inspection, not just aggregate win/loss --
experiments/replays/m6_public_v6fix_vs_v33_n20_*.replay):
defense_v6_encryptor_fix lost 0/20 (both seats) to this opponent, every game via
the same mechanism. The opponent commits a continuous, undying stream of
DEMOLISHER units (as many as its MP affords, EVERY turn from turn 4 onward, no
pauses/bursts) at a single fixed lane, chosen once per few turns based on which of
our corners it judges weaker. Our single-thickness corner turret
(`core_turret_anchors`'s `[26,12]` in the observed losses) gets rebuilt every turn
by `build_core_defense` after being destroyed, but a FRESH, zero-battle-tested
turret is then killed again in the SAME turn by concentrated fire from multiple
simultaneous demolishers before it can meaningfully intercept them (Verified directly
from spawn/death event pairs sharing the same unit id within one turn, e.g. turn 28:
unit '420' spawns at [26,12] and dies at [26,12] in the same turn). This is a
"reinforcements trickling in one at a time against a concentrated, sustained force"
failure: `reactive_defense`'s neighborhood backfill (one row behind the breach,
capped at `max_reactive_spend_per_turn` SP/turn) cannot outpace this because it
only adds MORE single-layer turrets in the same exposed spot, which die the same
way. Once that corner's wall+turret line is reliably dead every turn, subsequent
demolisher waves walk straight through to deal direct player-health breach damage
(`playerBreachDamage`), which is exactly what accumulates in the turn 27-35 health
trace (22 -> 3 HP) in every one of the 20 observed losses.

THE FIX (both changes, tested together -- see docs/MILESTONE_6_REPORT.md for the
before/after benchmark): (1) add a SECOND turret one tile further back at each of
the two extreme corners (`[2, 11]` behind `[1, 12]`, `[25, 11]` behind `[26, 12]`)
so a concentrated salvo has to kill roughly double the HP/firepower at that exact
choke point before breaching, instead of one turret dying to a single wave; (2)
raise `max_reactive_spend_per_turn` from 6 to 14 so the reactive-rebuild budget can
actually keep pace with a sustained (not just one-off) attack once a lane is
identified as contested, instead of being structurally capped below the rate the
opponent can re-attack. Both are corner/depth changes, not a hardcoded reaction to
this exact opponent's coordinates -- see the module's `_KNOWN WEAKNESS_` note below
each changed constant.
...
# Milestone 6 fix: was 6. A single-layer corner turret dying to a concentrated,
# sustained same-lane demolisher rush (see module docstring) and being rebuilt
# fresh every turn was not being reinforced fast enough at the old cap -- see
# docs/MILESTONE_6_REPORT.md for the before/after benchmark this was tuned against.
REACTIVE_DEFENSE_SP_CAP = 14
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def on_game_start(self, config):
        gamelib.debug_write('Configuring DEFENSE_V6_ENCRYPTOR_FIX candidate...')
        self.config = config
        ...
        self.max_reactive_spend_per_turn = REACTIVE_DEFENSE_SP_CAP  # SP cap on reactive rebuilding per turn
        ...
        # Milestone 6 fix: `[2, 11]` and `[25, 11]` are a NEW second layer, one tile
        # behind each extreme corner turret. Root cause (see module docstring):
        # a single-layer corner turret gets rebuilt every turn but is killed again
        # in the same turn by a concentrated, sustained multi-unit demolisher salvo
        # before it can hold the choke point, letting the lane go permanently
        # undefended. A second turret at that exact choke roughly doubles the
        # HP/firepower an attacker must overcome there before breaching, instead of
        # relying on a single unit that dies to one wave every turn indefinitely.
        self.core_turret_anchors = [
            [3, 11], [24, 11],      # KNOWN_WEAKNESS fix: corner depth turrets (M6).
            # NOTE: [1,12]/[26,12] used to be TURRET here; moved to
            # `core_corner_wall` below (see that field's comment) after two
            # rounds of empirical failure:
            #  1. [2,11]/[25,11] as a second TURRET layer -- rejected, those
            #     tiles are ALSO on the scoring edge (get_edge_locations
            #     BOTTOM_LEFT/BOTTOM_RIGHT), just as instantly breach-fatal as
            #     the corner itself. Only delayed the M6 loss ~8 turns.
            #  2. [3,11]/[24,11] (non-edge, true depth) as a second TURRET layer,
            #     PLUS a raised reactive-defense SP cap, PLUS MP-funded
            #     counter-offense -- all real, structural changes, but the
            #     opponent's continuously-growing (bit-ramp-fed) demolisher
            #     salvo still eventually overwhelms the corner's fixed SP
            #     rebuild budget every time (Verified across n=20 + n=12 runs,
            #     docs/MILESTONE_6_REPORT.md) -- a TURRET rebuilt fresh each
            #     turn keeps dying in the SAME turn it's rebuilt before it can
            #     contribute, same failure mode as v6, just delayed.
            # [3, 11]/[24, 11] are kept here as TURRETs (kill power, off the
            # scoring edge so dying there doesn't hand over a free breach) --
            # see `core_corner_wall` for the change that actually matters.
            [4, 12], [23, 12],
            [7, 11], [20, 11],
            [10, 10], [17, 10],
            [13, 10], [14, 10],
        ...
        # Milestone 6 fix: the two extreme corners [1,12]/[26,12] are THEMSELVES
        # scoring edge tiles (get_edge_locations BOTTOM_LEFT/BOTTOM_RIGHT) with
        # no further depth available behind them (diamond-board geometry tapers
        # to a 1-tile-wide sliver there -- Verified via gamelib.GameMap.
        # in_arena_bounds). A TURRET there (v6's design) costs 2 SP and buys 75
        # HP of tank, but this opponent's sustained salvo kills a fresh 75 HP
        # turret within the SAME turn it's rebuilt every time (Verified from
        # spawn/death event pairs sharing a unit id in one turn -- see module
        # docstring). A WALL costs only 1 SP (half the rebuild cost every time
        # this tile dies) and, critically, being ALIVE at all -- regardless of
        # remaining HP -- is what blocks enemy pathing onto that exact edge
        # tile; a WALL that's about to die still blocked pathing for the frames
        # it was alive, same as a TURRET would, but costs half as much SP to
        # keep re-blocking it turn after turn. The actual kill power moved to
        # the depth TURRETs in `core_turret_anchors` ([3,11]/[24,11]), which are
        # not themselves an instant breach if they die.
        self.core_corner_wall = [[1, 12], [26, 12]]
    ...
    def build_core_defense(self, game_state):
        ...
        game_state.attempt_spawn(WALL, self.core_corner_wall)
    ...
    def upgrade_core(self, game_state):
        ...
        game_state.attempt_upgrade(self.core_corner_wall)
    ...
    def opportunistic_offense(self, game_state):
        ...
        # Milestone 6 fix: this used to be `_stalemate_breaker`'s job alone, but
        # that helper explicitly skips itself once `self.ever_breached` is True
        # (see its docstring: it exists to break a MUTUAL stalemate, not to
        # counter-pressure an opponent who is actively grinding us down). Against
        # `public_opponents/travelling_salesmen_v33` that gating is exactly
        # backwards: we get breached early (turn ~4-25 depending on the fix
        # iteration) and then get ZERO extra offensive pressure for the rest of
        # the game (points_scored ended 1-30 in the unpatched loss -- see
        # docs/MILESTONE_6_REPORT.md), because MP alone (SP is untouched) --
        # this doesn't compete with the corner's structural rebuild budget.
        self._sustained_pressure_counter(game_state)
        self._stalemate_breaker(game_state)

    def _sustained_pressure_counter(self, game_state):
        """MP-funded counter-offense triggered specifically by evidence that WE
        are under sustained attack (a non-empty, still-decaying breach_history),
        independent of `ever_breached`/stalemate logic. Costs MP only (SP for
        structural rebuild is untouched), on a cadence so it doesn't cannibalize
        the normal scout probe's own MP floor."""
        turn = game_state.turn_number
        if not self.breach_history or turn % 3 != 0:
            return
        mp = game_state.get_resource(MP)
        demolisher_cost = game_state.type_cost(DEMOLISHER)[MP]
        if mp < demolisher_cost * 3:
            return
        best = self._get_cached_lane(game_state)
        num_demolishers = min(4, int(mp // demolisher_cost))
        game_state.attempt_spawn(DEMOLISHER, best, num_demolishers)
